<x-layout title="edit session">
    <main class="flex-grow container mx-auto px-6 py-10">
        <div class="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow">
            <h2 class="text-3xl font-bold mb-6 text-center">edit session</h2>

            @php
                $titleField = 'mb-4 bg-red-600';
            @endphp

            <form action="{{ route('sessions.update', $showtime->id) }}" method="post">
                @csrf
                @method('put')
                <div class="mb-5"></div>
                ...
                <div class="mb-5">
                    ...
                    <input type="time" name="start_time"
                        value="{{ old('start_time', $showtime->start_time?->format('H:i')) }}"
                ...
                <div class="mb-5">
                    ...
                    <input type="time" name="end_time"
                        value="{{ old('end_time', $showtime->end_time?->format('H:i')) }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('end_time')
                        <span class="text-red-500 text-sm font-bold block mt-1">{{ $message }}</span>
                    @enderror
                    ...
                    @foreach ($salons as $salon)
                        <label
                            ...
                            <input type="checkbox" name="salons[]" value="{{ $salon->id }}"
                                ...
                                {{ (is_array(old('salons')) && in_array($salon->id, old('salons'))) || $showtime->salons->contains($salon->id) ? 'checked' : '' }}>
