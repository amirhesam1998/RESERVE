<x-layout titile="single user">
    <div class="bg-white p-6 rounded-lg shadow">
        <h3 class="text-2xl font-semibold mb-2">{{ $user->first_name }} {{ $user->last_name }}</h3>
        <p class="text-gray-600 mb-4">{{ $user->email }}</p>
        <p class="text-gray-700">{{ $user->phone_number }}</p>

        <button class="mt-4 text-blue-600 hover:underline" action="posts/create">
            Read More
        </button>

        @can('salons', ['view-salons'])
            <a href="{{ route('salons.index') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100">
                Salons
            </a>
        @endcan

        @can('salons', ['create-salons'])
            <a href="{{ route('salons.create') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100">
                Create Salon
            </a>
        @endcan

        @can('users', [$user, 'edit-clients'])
            <a href="{{ route('users.edit', $user->id) }}"
                class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100">
                Edit Information
            </a>
        @endcan

        @can('users', [$user, 'editpass-clients'])
            <a href="{{ route('user.editpass.form', $user->id) }}"
                class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100">
                Edit Password
            </a>
        @endcan

        @can('users', [$user, 'delete-clients'])
            <form action="{{ route('users.destroy', $user->id) }}" method="post">
                @csrf
                @method('delete')
                <button type="submit" class="mt-4 text-blue-600 hover:underline">
                    Delete Acount
                </button>
            </form>
        @endcan

    </div>

    @if (session('success'))
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
            {{ session('success') }}
        </div>
    @endif

</x-layout>
