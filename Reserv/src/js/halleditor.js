...
async function loadSalonLayoutFromBackend(salonId) {
  try {
    ...
    // اگر سالن هنوز هیچ فضایی ذخیره نکرده (اولین بار است)، به دیفالت برنگرد
    ...
    floors = salon.floors.map((floor) => ({
      ...
      sections: (floor.sections || []).map((sec) => ({
        ...
        seats: (sec.seats || []).map((seat) => ({
          ...
          price: seat.price || 0,
          type: seat.type || "regular",
          customText: seat.custom_text ?? seat.customText ?? null,
        })),
      })),
    }));
...
async function exportToBackendJSON(salonId) {
  ...
  // --- ۲. فیلتر کردن و آماده‌سازی داده‌ها برای ارسال به بک‌اند ---
  const exportedFloors = floors.map((floor, index) => {
    return {
      id: floor.id,
      name: floor.name,
  ...
  // استفاده در هنگام ارسال درخواست fetch
