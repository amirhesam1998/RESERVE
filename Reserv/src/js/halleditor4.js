    ...
    currentSections.forEach((sec) => {
        ...
        sec.seats.forEach((seat) => {
            ...
            seatGroup.on("dragmove", (e) => {
                e.cancelBubble = true;
                const originalSeat = startPositions.find(
                    (p) => p.id === seat.id,
                );
                ...
                sectionGroup.getChildren().forEach((group) => {
                    if (selectedSeatIds.includes(group.id())) {
                        const startPos = startPositions.find(
                            (p) => p.id === group.id(),
                        );
                        if (startPos) {
                            if (group.id() === seat.id) {
                                ...
                                if (!e.evt.altKey) {
                                    followerX =
                                        Math.round(followerX / GRID_SIZE) *
                                        GRID_SIZE;
                                    followerY =
                                        Math.round(followerY / GRID_SIZE) *
                                        GRID_SIZE;
                                }
                                group.x(followerX);
                                group.y(followerY);
            ...
            seatGroup.on("dragend", (e) => {
                ...
                sectionGroup.getChildren().forEach((group) => {
                    if (selectedSeatIds.includes(group.id())) {
                        const targetSeat = sec.seats.find(
                            (s) => s.id === group.id(),
                        );
                        if (targetSeat) {
    ...
    if (currentSections.length > 0) {
        ...
        currentSections.forEach((sec) => {
            ...
            if (sec.seats && sec.seats.length > 0) {
                secSeatsMaxY =
                    Math.max(...sec.seats.map((s) => s.y)) + SEAT_SIZE;
            } else {
    ...
    floors.forEach((floor, index) => {
        ...
        item.addEventListener("dragover", (e) => {
            ...
            const items = Array.from(
                optionsList.querySelectorAll(".dropdown-item"),
            );
